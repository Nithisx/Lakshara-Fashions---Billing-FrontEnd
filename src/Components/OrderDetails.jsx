...
import { ArrowLeft, Calendar, User, Ruler, FileText, Printer, Check, Copy, AlertCircle, ShoppingBag, CreditCard, ExternalLink, RefreshCw, Eye } from 'lucide-react';
...
const OrderDetails = () => {
  ...
  const handleGenerateInvoice = async (e) => {
    ...
    const invoicePayload = {
      ...
      total_amount: parseFloat(order.stitching_price),
  ...
  if (loading) {
    return (
      <main className="flex-1 p-8 flex flex-col items-center justify-center bg-slate-50 text-slate-800 min-h-screen">
        ...
        <p className="mt-4 text-slate-400 text-sm font-medium">Loading order details...</p>
      </main>
    );
  }
  ...
  if (error || !order) {
    return (
      <main className="flex-1 p-8 max-w-md mx-auto flex flex-col items-center justify-center bg-slate-50 text-slate-800 min-h-screen text-center">
        ...
        <p className="text-slate-500 text-sm mt-2">{error || 'This order does not exist or has been removed.'}</p>
        <button
  ...
  const orderItemsList = typeof order.items === 'string' ? JSON.parse(order.items) : (order.items || []);
  const deliveryFormatted = new Date(order.delivery_date).toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' });
  const trialFormatted = order.trial_date ? new Date(order.trial_date).toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' }) : '—';
  const clothImagesList = order.cloth_images ? order.cloth_images.split('||') : [];
    ...
    <main className="flex-1 p-4 md:p-8 w-full bg-slate-50 text-slate-800 font-sans min-h-screen">
      ...
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 border-b border-slate-200 pb-6 w-full">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Stitching configurations, specifications, reference images, and billing status</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <span className={`inline-flex px-3 py-1 rounded-full text-xs font-extrabold border uppercase ${
            order.status === 'Pending' ? 'bg-amber-50 text-amber-705 border-amber-200' :
            order.status === 'In Progress' ? 'bg-blue-50 text-blue-705 border-blue-200' :
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 w-full">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm space-y-4">
            <h3 className="text-base font-bold text-slate-900 border-b border-slate-100 pb-2">Client Profile</h3>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div>
                <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider block">Full Name</span>
                <span className="font-semibold text-slate-850 block mt-0.5 capitalize">{order.customer_name}</span>
              </div>
              
              <div>
                <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider block">Phone Number</span>
                <span className="font-semibold text-slate-850 block mt-0.5">{order.customer_phone}</span>
              </div>

              {order.customer_email && (
                <div>
                  <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider block">Email Address</span>
                  <span className="font-semibold text-slate-850 block mt-0.5">{order.customer_email}</span>
                </div>
              )}

              <div>
                <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider block">Gender Profile</span>
                <span className="font-semibold text-slate-850 block mt-0.5">{order.customer_gender || 'Female'}</span>
              </div>
              ...
              {order.customer_address && (
                <div className="md:col-span-2">
                  ...
                  <span className="font-medium text-slate-650 block mt-1 leading-relaxed">
                    {order.customer_address}
                    {(order.customer_city || order.customer_state || order.customer_postal_code) && (
                      <span className="block text-slate-400 text-xs mt-0.5">
                        {[order.customer_city, order.customer_state, order.customer_postal_code].filter(Boolean).join(', ')}
                      </span>
                    )}
                  </span>
                </div>
                  ...
                  {orderItemsList.map((item, idx) => (
                    <React.Fragment key={item.id || idx}>
                      <tr className="hover:bg-slate-50/50 text-xs">
                        ...
                        <td className="py-3 px-4 text-center font-semibold text-slate-600">{item.quantity}</td>
                        <td className="py-3 px-4 text-right text-slate-600">₹{parseFloat(item.price).toFixed(2)}</td>
                        ...
                        <td className="py-3 px-4 text-right">
                          ...
                          >
                            ...
                            <span>{expandedItemIndices[idx] ? 'Hide' : 'Show Measurements'}</span>
                          </button>
                        </td>
                      </tr>
                        ...
                        <tr>
                          <td colSpan="6" className="bg-slate-50/50 p-4 border-t border-slate-100">
                            <div className="text-xs space-y-2">
                              <span className="font-bold text-slate-650 uppercase tracking-wider block">Measurements Parameters:</span>
                              {(!item.measurements || Object.keys(item.measurements).length === 0) ? (
                                <p className="text-slate-400 italic text-[11px]">No measurements configured for this item.</p>
                              ) : (
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                                  {Object.entries(item.measurements).map(([key, val]) => (
                                    <div key={key} className="bg-white border border-slate-200 p-2.5 rounded-lg">
          ...
          {clothImagesList.length > 0 && (
            <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm space-y-4">
              <h3 className="text-base font-bold text-slate-900 border-b border-slate-100 pb-2">Uploaded Reference Cloth Images</h3>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {clothImagesList.map((img, index) => (
                  ...
                  >
                    <img src={img} alt="cloth preview" className="w-full h-full object-cover group-hover:scale-105 transition duration-200" />
          ...
          {(order.special_instructions || order.inspiration_link) && (
            <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm space-y-4">
              <h3 className="text-base font-bold text-slate-900 border-b border-slate-100 pb-2">Order Specifications</h3>
              
              <div className="space-y-4 text-sm">
                {order.inspiration_link && (
                  <div>
                    <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider block">Inspiration Link</span>
                    <a 
                ...
                {order.special_instructions && (
                  <div>
                    <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider block">Special Instructions</span>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm space-y-4">
            <h3 className="text-base font-bold text-slate-900 border-b border-slate-100 pb-2">Scheduling timelines</h3>
            
            <div className="space-y-3.5 text-sm">
              <div className="flex justify-between">
                <span className="text-slate-500">Delivery Date</span>
                <span className="font-semibold text-slate-800">{deliveryFormatted}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Trial Date</span>
                <span className="font-semibold text-slate-800">{trialFormatted}</span>
              </div>
              <div className="flex justify-between border-t border-slate-100 pt-3">
                <span className="text-slate-500">Total Pieces</span>
                <span className="font-semibold text-slate-800">{order.quantity}</span>
              </div>
              {order.price_breakup && (
                <div className="border-t border-slate-100 pt-3">
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block mb-1">Pricing Breakdown Details</span>
                  <p className="text-xs text-slate-500 bg-slate-50 p-2 border border-slate-100 rounded-lg whitespace-pre-wrap">{order.price_breakup}</p>
                </div>
              )}
              <div className="flex justify-between border-t border-slate-200 pt-3.5">
                ...
                <span className="text-xl font-black text-indigo-600">₹{parseFloat(order.stitching_price).toFixed(2)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm space-y-4">
            <h3 className="text-base font-bold text-slate-900 border-b border-slate-100 pb-2">Boutique Invoice</h3>
            ...
            {order.invoice_id ? (
              ...
              <div className="space-y-4">
                <div className="bg-emerald-50 text-emerald-850 border border-emerald-200 rounded-xl p-3.5 text-center">
                  <span className="text-[10px] text-emerald-600 font-extrabold uppercase tracking-wider block">Linked Invoice</span>
                  <span className="text-sm font-mono font-bold block mt-1">{order.linked_invoice_number}</span>
                </div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  >
                    ...
                    <span>{copiedInvoice ? 'Link Copied' : 'Copy Invoice URL'}</span>
                  </button>
              ...
              <form onSubmit={handleGenerateInvoice} className="space-y-4">
                <span className="text-xs text-slate-500 leading-relaxed block">
                  This order has {orderItemsList.length} item(s) ready for invoicing. Select a payment method below to build and print the receipt:
                </span>
                ...
                >
                  ...
                  <span>{generatingInvoice ? 'Generating Invoice...' : 'Generate Invoice & Print'}</span>
                </button>
              </form>
            )}
          </div>
