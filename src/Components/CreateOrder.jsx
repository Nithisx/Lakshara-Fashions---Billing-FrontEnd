...
import { ArrowLeft, User, Ruler, Settings, Calendar, CreditCard, ShoppingBag, Plus, Trash2, Link, Upload, Search, X, Check, Eye } from 'lucide-react';
...
const CreateOrder = () => {
  ...
  const [viewingMeasurementsItemIndex, setViewingMeasurementsItemIndex] = useState(null);
  ...
  const handleAddCustomMeasurement = (e) => {
    ...
    if (itemMeasurements.some(m => m.key.toLowerCase() === customKey.trim().toLowerCase())) {
      alert('This parameter already exists.');
      return;
    }
    setItemMeasurements([...itemMeasurements, { key: customKey.trim(), value: customValue.trim() }]);
    setCustomKey('');
    setCustomValue('');
  };
  ...
  return (
    <main className="flex-1 p-4 md:p-8 w-full bg-slate-50 text-slate-800 font-sans min-h-screen">
      ...
      <header className="mb-8 border-b border-slate-200 pb-6 w-full">
        ...
        <p className="text-slate-500 text-sm mt-1">Configure customization details, stitching parameters, and client delivery date details</p>
      </header>
      ...
      {loadingCustomers ? (
        <div className="py-20 flex flex-col items-center justify-center w-full">
          ...
          <p className="mt-4 text-slate-400 text-sm font-medium">Fetching client profiles...</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 w-full">
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm relative">
              ...
              {selectedCustomer ? (
                ...
                <div className="flex items-center justify-between bg-indigo-50/50 border border-indigo-100 p-4 rounded-xl">
                  <div>
                    <div className="font-bold text-slate-800 capitalize text-sm">{selectedCustomer.full_name}</div>
                    <div className="text-xs text-slate-500 mt-1">Phone: {selectedCustomer.phone_number}</div>
                    {selectedCustomer.email && <div className="text-xs text-slate-500">Email: {selectedCustomer.email}</div>}
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                ...
                <div className="relative">
                    ...
                    <div className="absolute z-30 left-0 right-0 mt-2 bg-white border border-slate-200 rounded-xl shadow-lg max-h-60 overflow-y-auto divide-y divide-slate-100">
                      {filteredCustomers.length === 0 ? (
                        ...
                        filteredCustomers.map((c) => (
                          ...
                          >
                            <div>
                              <span className="font-bold text-slate-805 capitalize">{c.full_name}</span>
                              <span className="text-xs text-slate-500 block mt-0.5">{c.phone_number}</span>
                            </div>
                  ...
                  <div className="flex justify-end mt-2">
                    ...
                    >
                      + Add a new customer to directory first
                    </button>
                  </div>
                </div>
              )}
            ...
            <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm space-y-6">
              <h3 className="text-base font-bold text-slate-900 border-b border-slate-100 pb-2">Add Items to this Order</h3>

              {/* Add Item Form */}
              <div className="bg-slate-50 border border-slate-200 p-6 rounded-2xl space-y-4">
                <span className="text-xs font-extrabold text-slate-700 uppercase tracking-wider block">Item Builder Configuration</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  {/* Unit price */}
                  <div>
                    <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Stitching Price (₹) *</label>
                    <div className="relative">
                ...
                <div className="pt-2">
                  <button
                    type="button"
                    onClick={() => setShowMeasurementsSection(!showMeasurementsSection)}
                  ...
                  >
                    ...
                    <span>{showMeasurementsSection ? 'Hide Measurements Box' : 'Add Measurements for this Item'}</span>
                  </button>
                </div>
                ...
                {showMeasurementsSection && (
                  <div className="bg-white border border-slate-200 p-4 rounded-xl space-y-4 animate-fadeIn">
                    <span className="text-xs font-bold text-slate-700 uppercase tracking-wider block border-b border-slate-100 pb-1">Measurements Workspace</span>
                    
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      {itemMeasurements.map((item, idx) => (
                        <div key={item.key} className="flex flex-col gap-1">
                          ...
                          <input
                            ...
                            value={item.value}
                            onChange={(e) => handleMeasurementValChange(idx, e.target.value)}
              ...
              <div className="border border-slate-200 rounded-xl overflow-hidden">
                ...
                {items.length === 0 ? (
                  <div className="py-8 text-center text-slate-400 text-xs bg-white">
                    No items added yet. Complete the Item Builder configuration above to list pieces.
                  </div>
                ) : (
                          ...
                          <React.Fragment key={item.id}>
                            <tr className="hover:bg-slate-50/50">
                              ...
                              <td className="py-3 px-4 text-right space-x-2">
                                <button
                                  type="button"
                                  onClick={() => setViewingMeasurementsItemIndex(viewingMeasurementsItemIndex === idx ? null : idx)}
                                  className="text-indigo-650 hover:text-indigo-700 font-semibold inline-flex items-center gap-1"
                                >
                                  <Eye className="w-3.5 h-3.5" />
                                  <span>{viewingMeasurementsItemIndex === idx ? 'Hide' : 'Measurements'}</span>
                                </button>
                                <button
                                  type="button"
                              ...
                              <tr>
                                <td colSpan="6" className="bg-slate-50/50 p-4 border-t border-slate-100">
                                  <div className="text-xs space-y-2">
                                    <span className="font-bold text-slate-700 uppercase tracking-wider block">Measurements Parameters:</span>
                                    {Object.keys(item.measurements).length === 0 ? (
                                      ...
                                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                                        {Object.entries(item.measurements).map(([key, val]) => (
                                          <div key={key} className="bg-white border border-slate-200 p-2 rounded-lg">
            ...
            <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm space-y-6">
              <h3 className="text-base font-bold text-slate-900 border-b border-slate-100 pb-2">Reference Material & Instructions</h3>
                ...
                <div className="flex items-center justify-center w-full">
                  <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-slate-200 border-dashed rounded-xl cursor-pointer bg-slate-55 hover:bg-slate-100/50 transition">
                    <div className="flex flex-col items-center justify-center pt-5 pb-6 text-slate-500">
                      ...
                      <p className="text-xs font-medium"><span className="font-semibold text-indigo-600">Click to upload cloth references</span> or drag and drop</p>
                      <p className="text-[10px] text-slate-400 mt-1">PNG, JPG or JPEG images</p>
                    </div>
                    <input
                      type="file"
                      className="hidden"
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-6 gap-3 mt-4">
                    {clothImages.map((img, idx) => (
                      <div key={idx} className="relative group aspect-square rounded-lg overflow-hidden border border-slate-200">
                        <img src={img} alt="cloth references" className="w-full h-full object-cover" />
                        <button
                          type="button"
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-white border border-slate-200 p-6 rounded-2xl space-y-5 shadow-sm">
              <h3 className="text-base font-bold text-slate-900 border-b border-slate-100 pb-3">Scheduling timelines</h3>
            ...
            <div className="bg-white border border-slate-200 p-6 rounded-2xl space-y-5 shadow-sm">
              <h3 className="text-base font-bold text-slate-900 border-b border-slate-100 pb-3">Summary & Price Details</h3>
              ...
              <div className="border-t border-slate-100 pt-4 space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-slate-800">{totalPieces}</span>
                </div>
                <div className="flex justify-between border-t border-slate-100 pt-2.5">
                  <span className="font-bold text-slate-900 text-base">Grand Total</span>
                  <span className="text-xl font-black text-indigo-600">₹{totalOrderPrice.toFixed(2)}</span>
                </div>
              </div>
              ...
              >
                ...
                <span>{loading ? 'Submitting Order...' : 'Register Stitching Order'}</span>
              </button>
            </div>
